...
import { Shield, ChevronRight, CheckCircle, Upload, X, FileText } from 'lucide-react'
...
function IndividualRegisterForm() {
  ...
  function handleFileSelect(file: File) {
    ...
    if (file.type.startsWith('image/')) {
      ...
      setIdPreview(null) // PDF — show filename only
    }
  }
  ...
  function handleSubmit() {
    // Persist credentials in sessionStorage so the login page can authenticate in demo mode
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 500 }}>
          ...
          <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.75, marginBottom: 24 }}>
            Your identity verification is under review. You'll receive a confirmation
            on <strong>{form.email || form.phone}</strong> within 24–48 hours.
          </p>

          {/* Demo mode login hint */}
          {form.email && (
            <div style={{ background: 'var(--gold-pale)', border: '1px solid var(--gold-border)', borderRadius: 10, padding: '16px 20px', marginBottom: 28, textAlign: 'left' }}>
              <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 8 }}>
                Your Login Credentials
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                  <span style={{ color: 'var(--text-muted)' }}>Email</span>
                  <strong style={{ color: 'var(--text)', fontFamily: 'monospace' }}>{form.email}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                  <span style={{ color: 'var(--text-muted)' }}>Password</span>
                  <strong style={{ color: 'var(--text)', fontFamily: 'monospace' }}>{'•'.repeat(form.password.length)}</strong>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Link href="/dashboard" className="btn btn-outline-dark">Go to Demo Dashboard</Link>
          </div>
        </div>
      </div>
    )
          ...
          {STEPS.map((s, i) => (
            <div key={s.n} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: step === s.n ? 600 : 400, color: step === s.n ? 'var(--text)' : 'var(--text-muted)' }}>
                  {s.label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
        ...
        <div className="card" style={{ padding: '40px 36px' }}>
          ...
          {step === 3 && (
            <div className="fade-in">
              ...
              <div style={{ marginBottom: 24 }}>
                <label className="label">Upload ID Document <span style={{ color: 'var(--risk-high)', marginLeft: 2 }}>*</span></label>
                ...
                {idFile ? (
                  ...
                  <div style={{ border: '2px solid var(--forest-lt)', borderRadius: 10, overflow: 'hidden', background: 'var(--forest-pale)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px' }}>
                      <div>
                        ...
                        <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{(idFile.size / 1024).toFixed(0)} KB · {idFile.type.split('/')[1].toUpperCase()}</p>
                      </div>
                      <button
                        type="button"
                        onClick={removeFile}
                        style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 600, color: 'var(--risk-high)', background: 'var(--risk-high-bg)', border: 'none', borderRadius: 6, padding: '5px 10px', cursor: 'pointer' }}
                      >
                  ...
                  <div
                    ...
                    onDrop={handleDrop}
                    style={{
                      border: `2px dashed ${dragOver ? 'var(--forest-mid)' : 'var(--border)'}`,
                      borderRadius: 10,
                      padding: '36px 20px',
                      textAlign: 'center',
                      cursor: 'pointer',
                      background: dragOver ? 'var(--forest-pale)' : 'var(--surface)',
                      transition: 'border-color .15s, background .15s',
                    }}
                  >
                    <Upload className="w-7 h-7 mx-auto mb-3" style={{ color: dragOver ? 'var(--forest-mid)' : 'var(--text-faint)' }} />
                    <p style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-mid)', marginBottom: 4 }}>
                      {dragOver ? 'Drop to upload' : 'Drop your ID here or click to browse'}
                    </p>
          ...
          {step === 4 && (
            <div className="fade-in">
                ...
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', padding: '13px 18px', borderBottom: i < arr.length - 1 ? '1px solid var(--border-lt)' : 'none', gap: 16 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>
                      {row.label === 'ID Document' && idFile
                        ? <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><CheckCircle style={{ width: 13, height: 13, color: 'var(--risk-low)' }} />{row.value}</span>
                        : row.value}
                    </span>
                  </div>
                ))}
              ...
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: 'var(--text-muted)', marginBottom: 24, cursor: 'pointer' }}>
                ...
                I confirm the information above is accurate and I agree to TrustNet's{' '}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, paddingTop: 24, borderTop: '1px solid var(--border)' }}>
            ...
            {step < 4
              ...
              : <button className="btn btn-gold" onClick={handleSubmit}>Submit Registration →</button>
            }
          </div>
        </div>
      </div>
